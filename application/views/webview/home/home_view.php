<!-- ======= Hero Section ======= -->
<section id="hero" class="hero d-flex align-items-center">

    <div class="container" data-aos="zoom-out" data-aos-delay="100">
        <!-- <div class="row"> -->
        <div class="wrapper-body">
            <div class="herotext textcol-xl-6">
                <h1>Bersama Alumni Kita </h1>
                <h1 class="pb-4"> Akan Membangun Negeri</h1>
                <!-- <p>Closest Event:</p>
                    <p>“Alumni Berkumpul dalam Rangka” | 23 Februari 2023 | Jakarta Selatan
                    </p> -->
                <a href="<?php echo site_url() ?>information/newest_type?cat_n=Kabar Alumni" class="btn btn-white-side mt-3 font-weight-bold" type="submit">Kabar Alumni
                    <span class="white-dot">
                        <span class="yellow-dot">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 1024 1024">
                                <path fill="white" d="M338.752 104.704a64 64 0 0 0 0 90.496l316.8 316.8l-316.8 316.8a64 64 0 0 0 90.496 90.496l362.048-362.048a64 64 0 0 0 0-90.496L429.248 104.704a64 64 0 0 0-90.496 0z" />
                            </svg>
                        </span>
                    </span>
                </a>
                <a href="<?php echo site_url('event') ?>" class="btn btn-white-side mt-3" type="submit">Acara
                    <span class="white-dot">
                        <span class="yellow-dot">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 1024 1024">
                                <path fill="white" d="M338.752 104.704a64 64 0 0 0 0 90.496l316.8 316.8l-316.8 316.8a64 64 0 0 0 90.496 90.496l362.048-362.048a64 64 0 0 0 0-90.496L429.248 104.704a64 64 0 0 0-90.496 0z" />
                            </svg>
                        </span>
                    </span>
                </a>
                <a href="<?php echo site_url() ?>information/newest_type?cat_n=Lowongan Kerja" class="btn btn-white-side mt-3" type="submit">Lowongan Kerja
                    <span class="white-dot">
                        <span class="yellow-dot">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 1024 1024">
                                <path fill="white" d="M338.752 104.704a64 64 0 0 0 0 90.496l316.8 316.8l-316.8 316.8a64 64 0 0 0 90.496 90.496l362.048-362.048a64 64 0 0 0 0-90.496L429.248 104.704a64 64 0 0 0-90.496 0z" />
                            </svg>
                        </span>
                    </span>
                </a>
                <a href="<?php echo site_url() ?>information/newest_type?cat_n=Beasiswa" class="btn btn-white-side mt-3" type="submit">Info Beasiswa
                    <span class="white-dot">
                        <span class="yellow-dot">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 1024 1024">
                                <path fill="white" d="M338.752 104.704a64 64 0 0 0 0 90.496l316.8 316.8l-316.8 316.8a64 64 0 0 0 90.496 90.496l362.048-362.048a64 64 0 0 0 0-90.496L429.248 104.704a64 64 0 0 0-90.496 0z" />
                            </svg>
                        </span>
                    </span>
                </a>
            </div>
            <div class="pt-5">
                <button class="btn btn-follow-us mt-1 text-white" type="submit">Follow Us <span class="ms-2">-</span>
                    <svg class="mx-3" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256">
                        <g fill="none">
                            <rect width="256" height="256" fill="url(#skillIconsInstagram0)" rx="60" />
                            <rect width="256" height="256" fill="url(#skillIconsInstagram1)" rx="60" />
                            <path fill="#fff" d="M128.009 28c-27.158 0-30.567.119-41.233.604c-10.646.488-17.913 2.173-24.271 4.646c-6.578 2.554-12.157 5.971-17.715 11.531c-5.563 5.559-8.98 11.138-11.542 17.713c-2.48 6.36-4.167 13.63-4.646 24.271c-.477 10.667-.602 14.077-.602 41.236s.12 30.557.604 41.223c.49 10.646 2.175 17.913 4.646 24.271c2.556 6.578 5.973 12.157 11.533 17.715c5.557 5.563 11.136 8.988 17.709 11.542c6.363 2.473 13.631 4.158 24.275 4.646c10.667.485 14.073.604 41.23.604c27.161 0 30.559-.119 41.225-.604c10.646-.488 17.921-2.173 24.284-4.646c6.575-2.554 12.146-5.979 17.702-11.542c5.563-5.558 8.979-11.137 11.542-17.712c2.458-6.361 4.146-13.63 4.646-24.272c.479-10.666.604-14.066.604-41.225s-.125-30.567-.604-41.234c-.5-10.646-2.188-17.912-4.646-24.27c-2.563-6.578-5.979-12.157-11.542-17.716c-5.562-5.562-11.125-8.979-17.708-11.53c-6.375-2.474-13.646-4.16-24.292-4.647c-10.667-.485-14.063-.604-41.23-.604h.031Zm-8.971 18.021c2.663-.004 5.634 0 8.971 0c26.701 0 29.865.096 40.409.575c9.75.446 15.042 2.075 18.567 3.444c4.667 1.812 7.994 3.979 11.492 7.48c3.5 3.5 5.666 6.833 7.483 11.5c1.369 3.52 3 8.812 3.444 18.562c.479 10.542.583 13.708.583 40.396c0 26.688-.104 29.855-.583 40.396c-.446 9.75-2.075 15.042-3.444 18.563c-1.812 4.667-3.983 7.99-7.483 11.488c-3.5 3.5-6.823 5.666-11.492 7.479c-3.521 1.375-8.817 3-18.567 3.446c-10.542.479-13.708.583-40.409.583c-26.702 0-29.867-.104-40.408-.583c-9.75-.45-15.042-2.079-18.57-3.448c-4.666-1.813-8-3.979-11.5-7.479s-5.666-6.825-7.483-11.494c-1.369-3.521-3-8.813-3.444-18.563c-.479-10.542-.575-13.708-.575-40.413c0-26.704.096-29.854.575-40.396c.446-9.75 2.075-15.042 3.444-18.567c1.813-4.667 3.983-8 7.484-11.5c3.5-3.5 6.833-5.667 11.5-7.483c3.525-1.375 8.819-3 18.569-3.448c9.225-.417 12.8-.542 31.437-.563v.025Zm62.351 16.604c-6.625 0-12 5.37-12 11.996c0 6.625 5.375 12 12 12s12-5.375 12-12s-5.375-12-12-12v.004Zm-53.38 14.021c-28.36 0-51.354 22.994-51.354 51.355c0 28.361 22.994 51.344 51.354 51.344c28.361 0 51.347-22.983 51.347-51.344c0-28.36-22.988-51.355-51.349-51.355h.002Zm0 18.021c18.409 0 33.334 14.923 33.334 33.334c0 18.409-14.925 33.334-33.334 33.334c-18.41 0-33.333-14.925-33.333-33.334c0-18.411 14.923-33.334 33.333-33.334Z" />
                            <defs>
                                <radialGradient id="skillIconsInstagram0" cx="0" cy="0" r="1" gradientTransform="matrix(0 -253.715 235.975 0 68 275.717)" gradientUnits="userSpaceOnUse">
                                    <stop stop-color="#FD5" />
                                    <stop offset=".1" stop-color="#FD5" />
                                    <stop offset=".5" stop-color="#FF543E" />
                                    <stop offset="1" stop-color="#C837AB" />
                                </radialGradient>
                                <radialGradient id="skillIconsInstagram1" cx="0" cy="0" r="1" gradientTransform="matrix(22.25952 111.2061 -458.39518 91.75449 -42.881 18.441)" gradientUnits="userSpaceOnUse">
                                    <stop stop-color="#3771C8" />
                                    <stop offset=".128" stop-color="#3771C8" />
                                    <stop offset="1" stop-color="#60F" stop-opacity="0" />
                                </radialGradient>
                            </defs>
                        </g>
                    </svg>
                    <svg class="me-3" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256">
                        <path fill="#1877F2" d="M256 128C256 57.308 198.692 0 128 0C57.308 0 0 57.307 0 128c0 63.888 46.808 116.843 108 126.445V165H75.5v-37H108V99.8c0-32.08 19.11-49.8 48.347-49.8C170.352 50 185 52.5 185 52.5V84h-16.14C152.958 84 148 93.867 148 103.99V128h35.5l-5.675 37H148v89.445c61.192-9.602 108-62.556 108-126.445" />
                        <path fill="#FFF" d="m177.825 165l5.675-37H148v-24.01C148 93.866 152.959 84 168.86 84H185V52.5S170.352 50 156.347 50C127.11 50 108 67.72 108 99.8V128H75.5v37H108v89.445A128.959 128.959 0 0 0 128 256a128.9 128.9 0 0 0 20-1.555V165h29.825" />
                    </svg>
                    <svg class="me-2" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 180">
                        <path fill="red" d="M250.346 28.075A32.18 32.18 0 0 0 227.69 5.418C207.824 0 127.87 0 127.87 0S47.912.164 28.046 5.582A32.18 32.18 0 0 0 5.39 28.24c-6.009 35.298-8.34 89.084.165 122.97a32.18 32.18 0 0 0 22.656 22.657c19.866 5.418 99.822 5.418 99.822 5.418s79.955 0 99.82-5.418a32.18 32.18 0 0 0 22.657-22.657c6.338-35.348 8.291-89.1-.164-123.134Z" />
                        <path fill="#FFF" d="m102.421 128.06l66.328-38.418l-66.328-38.418z" />
                    </svg>
                </button>
            </div>
        </div>
        <!-- </div> -->
</section><!-- End Hero -->

<!-- Section About card -->
<section class="aboutcards ">
    <div class="aboutcard container position-relative " data-aos="zoom-in" data-aos-delay="100">
        <div class="selengkapnya">
            <div class="d-flex row align-content-between">
                <div class="text col-md-6 ">
                    <h3 data-aos="zoom-in-up" data-aos-offset="300" data-aos-easing="ease-in-sine">Dengan bangga kami sampaikan</h3>
                </div>
                <div class=" btn-about col-md-6 ml-10" data-aos="zoom-in-up" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <a href="<?php echo site_url('dataAlumni'); ?>" class="btn btn-get-about scrollto">Selengkapnya<svg class="carets" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 256 256">
                            <path fill="currentColor" d="m180.24 132.24l-80 80a6 6 0 0 1-8.48-8.48L167.51 128L91.76 52.24a6 6 0 0 1 8.48-8.48l80 80a6 6 0 0 1 0 8.48Z" />
                        </svg></a>
                </div>
            </div>
        </div>
        <!-- <div class="aset3 img-fluid">
            <img src="<?php echo base_url(); ?>assets/image/Group_103.png" alt="">
        </div> -->
        <div class="row">
            <div class="col-md-6 col-lg-3" data-aos="zoom-in-up" data-aos-offset="300" data-aos-easing="ease-in-sine">
                <div class="card cardabout">
                    <h5 class="card-title"><?= $collective->ttl_school ?></h5>
                    <p class="card-text">Sekolah</p>
                    <div class="hover-info">
                        <?php
                        $this->db->select('school_name');
                        $this->db->from('mast_school a');
                        $this->db->where('active', 1);
                        $sekolah = $this->db->get()->result();
                        ?>
                        <p>
                            <?php
                            $count = count($sekolah); // Get the total number of elements in $sekolah array
                            foreach ($sekolah as $key => $s) {
                                echo $s->school_name;
                                if ($key < $count - 1) { // Check if it's not the last iteration
                                    echo " dan ";
                                }
                            }
                            ?>
                        </p>
                        <!-- <p>Sukoharjo dan Medan</p> -->
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-3" data-aos="zoom-in-up" data-aos-offset="300" data-aos-easing="ease-in-sine">
                <div class="card cardabout">
                    <h5 class="card-title"><?= $collective->ttl_generation ?></h5>
                    <p class="card-text">Angkatan</p>
                    <div class="hover-info">
                        <p>Akan Terus Bertambah</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-3" data-aos="zoom-in-up" data-aos-offset="300" data-aos-easing="ease-in-sine">
                <div class="card cardabout">
                    <h5 class="card-title"><?= $collective->ttl_graduate ?></h5>
                    <p class="card-text">Lulusan</p>
                    <div class="hover-info">
                        <p>Tersebar Ke Berbagai Perguruan Tinggi</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-3" data-aos="zoom-in-up" data-aos-offset="300" data-aos-easing="ease-in-sine">
                <div class="card cardabout">
                    <h5 class="card-title text-light"><?= $collective->ttl_superior ?></h5>
                    <p class="card-text">Lulusan Unggulan</p>
                    <div class="hover-info">
                        <p>Jalur Undangan dan Universitas Luar Negeri</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </div>
</section>
<!-- end of Section About card -->


<main id="main">
    <!-- ======= Tabs Section ======= -->
    <section id="tabs" class="tabs">
        <div class="container" data-aos="fade-up">
            <div class="tab-content">
                <div class="tab-pane active show" id="tab-1">
                    <div class="row">
                        <div class="col-lg-6  text-center" data-aos="fade-up" data-aos-delay="200">
                            <img src="<?php echo base_url(); ?>assets/image/wisuda_smau.png" alt="" class="img-fluid" style="border-radius: 30px;">
                        </div>
                        <div class="textall col-lg-6 mt-3 mt-lg-0" data-aos="fade-up" data-aos-delay="100">
                            <p class="texto">Tentang Platform</p>
                            <h1 class=" texts">
                                APA ITU CTARSA ALUMNI?
                            </h1>
                            <p class="textp">
                                Ikatan CTARSA Alumni adalah sebuah platform yang memiliki peran penting dalam menghubungkan para alumni. Melalui platform ini, alumni dapat saling terhubung dan berbagi berbagai informasi, pengalaman, dan kenangan mereka selama menjalani masa sekolah di CTARSA. Ini bukan hanya sekadar wadah, tetapi juga menjadi jembatan emosional yang memungkinkan para alumni merasakan nostalgia akan masa-masa indah mereka bersama. Mari jadikan wadah ini menjadi penuh dengan informasi yang bermanfaat.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section><!-- End Tabs Section -->

    <!-- ======= TimelineSection ======= -->
    <section id="timeline" class="timeline ">
        <div class="container1">
            <div class="aset1 col-3">
                <img src="<?php echo base_url(); ?>assets/image/Vector.png" alt="">
            </div>
            <div class="row text-center justify-content-center mb-5">
                <div class="col-xl-6 col-lg-8">
                    <h2 class="font-weight-bold ">Sejarah Singkat</h2>
                </div>
            </div>

            <div class="row">
                <div class="col">
                    <div class="timeline-steps aos-init aos-animate" data-aos="fade-up">
                        <div class="timeline-step" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                            <div class="timeline-content" data-toggle="popover" data-trigger="hover" data-placement="top" title="" data-content="And here's some amazing content. It's very engaging. Right?" data-original-title="2003">
                                <div class="bgtimeline "> <img src="<?php echo base_url(); ?>assets/image/2004.png" alt="" />
                                </div>
                                <p class="h6 mt-3 mb-1">2004</p>
                                <p class="h6 mb-0 mb-lg-0 text-light">Bencana Tsunami melanda Nangroe Aceh Darussalam</p>
                                <div class="popup text-light">
                                    Selengkapnya
                                    <div class="popup-text">
                                        <p>Bencana Tsunami melanda Nangroe Aceh Darussalam mengakibatkan kurang lebih 200.000 orang meninggal dunia dan hilang. Bencana ini juga mengakibatkan banyak anak yatim piatu dan terlantar. Hal ini yang mengugah Chairul Tanjung dan Anita Ratnasari Tanjung mengagas Rumah Anak Madani.</p>
                                        <img src="<?php echo base_url(); ?>assets/image/image82.png" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="timeline-step" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                            <div class="timeline-content" data-toggle="popover" data-trigger="hover" data-placement="top" title="" data-content="And here's some amazing content. It's very engaging. Right?" data-original-title="2004">
                                <div class="bgtimeline"> <img src="<?php echo base_url(); ?>assets/image/2005.png" alt="" />
                                </div>
                                <p class="h6 mt-3 mb-1">2005</p>
                                <p class="h6  mb-0 mb-lg-0 text-light">Didirikan Rumah Anak Madani di Deli Serdang</p>
                                <div class="popup text-light">
                                    Selengkapnya
                                    <div class="popup-text">
                                        <p>Rumah Anak Madani di Deli Serdang, Sumatera Utara didirikan untuk membantu anak-anak korban Tsunami Aceh. Mengayomi anak-anak untuk dapat bangkit peduli pada pendidikan dan mampu menatap masa depan yang lebih cerah.</p>
                                        <img src="<?php echo base_url(); ?>assets/image/image87.png" alt="" />
                                        <img src="<?php echo base_url(); ?>assets/image/image88.png" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="timeline-step" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                            <div class="timeline-content" data-toggle="popover" data-trigger="hover" data-placement="top" title="" data-content="And here's some amazing content. It's very engaging. Right?" data-original-title="2005">
                                <div class="bgtimeline"> <img src="<?php echo base_url(); ?>assets/image/2010.png" alt="" />
                                </div>
                                <p class="h6 mt-3 mb-1">2010</p>
                                <p class="h6  mb-0 mb-lg-0 text-light">Dikembangkan menjadi SMA Unggulan CT ARSA Foundations</p>
                                <div class="popup text-light">
                                    Selengkapnya
                                    <div class="popup-text">
                                        <p>Rumah Anak Madani dikembangkan menjadi Sekolah Menengah Atas Unggulan CT foundation. Siswa-siswi direkrut berdasarkan dari keluarga tidak mampu dan berprestasi. Hingga kini lebih dari 1000 anak mengayom pendidikan di SMA UNGGULAN CT foundation.</p>
                                        <img src="<?php echo base_url(); ?>assets/image/image85.png" alt="" />
                                        <img src="<?php echo base_url(); ?>assets/image/image86.png" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="timeline-step" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                            <div class="timeline-content" data-toggle="popover" data-trigger="hover" data-placement="top" title="" data-content="And here's some amazing content. It's very engaging. Right?" data-original-title="2010">
                                <div class="bgtimeline"> <img src="<?php echo base_url(); ?>assets/image/2015.png" alt="" />
                                </div>
                                <p class="h6 mt-3 mb-1">2015</p>
                                <p class="h6 mb-0 mb-lg-0 text-light">Open New Branch</p>
                                <div class="popup text-light">
                                    Selengkapnya
                                    <div class="popup-text1">
                                        <p>Bersama Yayasan Mitra Netra, CT ARSA FOUNDATION bertekad untuk meningkatkan kualitas dan partisipasi tuna netra dibidang pendidikan dan pekerjaan sehingga tuna netra dapat hidup mandiri, cerdas dan bermakna di tengah masyarakat.</p>
                                        <img src="<?php echo base_url(); ?>assets/image/image89.png" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="timeline-step mb-0" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                            <div class="timeline-content" data-toggle="popover" data-trigger="hover" data-placement="top" title="" data-content="And here's some amazing content. It's very engaging. Right?" data-original-title="2020">
                                <div class="bgtimeline"> <img src="<?php echo base_url(); ?>assets/image/2018.png" alt="" />
                                </div>
                                <p class="h6 mt-3 mb-1">2018</p>
                                <p class="h6 mb-0 mb-lg-0 text-light">Didirikan SMA Unggulan CT ARSA Foundation di Sukoharjo</p>
                                <div class="popup text-light">
                                    Selengkapnya
                                    <div class="popup-text1">
                                        <p>SMA Unggulan CT ARSA Foundation Sukoharjo Sesuai dengan misinya untuk memutus mata rantai kemiskinan melalui pendidikan yang berkualitas dan kesehatan yang optimal kembali diresmikan sekolah unggulan dengan fasilitas yang lebih lengkap dengan jumlah 200 siswa/i per-angkatan..</p>
                                        <img src="<?php echo base_url(); ?>assets/image/image90.png" alt="" />
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="aset2 d-flex justify-content-end mb-10">
                <img src="<?php echo base_url(); ?>assets/image/Group 104.png" alt="">
            </div>
        </div>
    </section><!-- End Services Section -->

    <!-- ======= Testimoni 2 Section ======= -->
    <section id="testimoni" class="testimoni">
        <div class="container" data-aos="fade-up">
            <div class="gtco-testimonials">
                <div class="testimoniheader">
                    <p class="pt-5">Testimoni</p>
                    <h2>APA KATA ALUMNI</h2>
                </div>
                <div class="owl-carousel owl-carousel1 owl-theme">
                    <div>
                        <div class="card text-center">
                            <div class="image-container">
                                <div class="video">
                                    <img class="card-img-top" src="assets/testimoni_mis.PNG" alt="testi" />
                                    <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="https://www.youtube.com/embed/yjhM9C3eiJQ" data-bs-target="#videoModal">
                                        <span></span>
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <h5>MUHAMMAD IQBAL SAPUTRA <br /></h5>
                                <p class="card-text">
                                    “ Halo perkenalkan nama saya Muhammad Iqbal Saputra. Saya dari SMA
                                    unggulan CT Arsa Foundation Sukoharjo angkatan pertama dan sekarang saya
                                    berku... ”
                                </p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="card text-center">
                            <div class="image-container">
                                <div class="video">
                                    <img class="card-img-top" src="assets/testimoni_fdya.PNG" alt="testi" />
                                    <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="https://www.youtube.com/embed/ukikC8Jm2r4" data-bs-target="#videoModal">
                                        <span></span>
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <h5>FIRDA DIAN YUDHA AZ ZAHRA <br /></h5>
                                <p class="card-text">
                                    “ Assalaikum warahmatullahi wabarakatuh. Perkenalkan saya Firda Dian Azzahra selaku salah satu alumnus dari SMA Unggulan CT Arsa Foundation Sukoharjo... ”
                                </p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="card text-center">
                            <div class="image-container">
                                <div class="video">
                                    <img class="card-img-top" src="assets/testimoni_ahw.PNG" alt="testi" />
                                    <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="https://www.youtube.com/embed/HgOysrXBDCg" data-bs-target="#videoModal">
                                        <span></span>
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <h5>ADNAN HASYIM WIBOWO <br /></h5>
                                <p class="card-text">
                                    “ Halo semuanya saya Adnan Hasyim salah satu alumni angkatan pertama SMA Unggulan CTARSA Foundation Sukoharjo dan sekarang sedang melanjutkan studi d... ”
                                </p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="card text-center">
                            <div class="image-container">
                                <div class="video">
                                    <img class="card-img-top" src="assets/testimoni_tar.PNG" alt="testi" />
                                    <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="https://www.youtube.com/embed/okSZYkn8rco" data-bs-target="#videoModal">
                                        <span></span>
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <h5>TRI ARDIAN RINALDA <br /></h5>
                                <p class="card-text">
                                    “ Assalamualaikum warahmatullahi wabarakatuh. Halo teman-teman semuanya perkenalkan nama Aku Tri Ardian Rinalda dan biasa dipanggil Ardi. Aku alumni ... ”
                                </p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="card text-center">
                            <div class="image-container">
                                <div class="video">
                                    <img class="card-img-top" src="assets/testimoni_hir.PNG" alt="testi" />
                                    <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="https://www.youtube.com/embed/KJoq6ZKTMrU" data-bs-target="#videoModal">
                                        <span></span>
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <h5>HILWA IFFATUR RAMADHANI <br /></h5>
                                <p class="card-text">
                                    “ Assalamualaikum warahmatullahi wabarakatuh. Halo teman-teman semua saya Hilwa Iffatur Ramadhani alumni angkatan pertama SMA Unggulan CT Arsa Founda... ”
                                </p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="card text-center">
                            <div class="image-container">
                                <div class="video">
                                    <img class="card-img-top" src="assets/testimoni_djna.PNG" alt="testi" />
                                    <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="https://www.youtube.com/embed/T-0PyWkzApQ" data-bs-target="#videoModal">
                                        <span></span>
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <h5>DAVIN JENNY NUR ANANTHA <br /></h5>
                                <p class="card-text">
                                    “ Halo perkenalkan saya Davin Jenny Nur Ananta alumni angkatan ke-2 SMA Unggulan CT ARSA Foundation Sukoharjo. Saat ini saya adalah mahasiswa Fakulta... ”
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                <!--
                    <div class="d-flex justify-content-center">
                    <a href="<?php echo site_url('testimoni') ?>" class="btn btn-selengkapnya3">Lihat semua</a>
                </div>
                -->
            </div>

        </div>
    </section>
    <!-- End of testimonial 2 Section -->

    <!-- ======= Testimoni Section ======= -->
    <!--    <section id="testimoni" class="testimoni">
        <div class="container" data-aos="fade-up">
            <div class="gtco-testimonials">
                <div class="testimoniheader">
                    <p class="pt-5">Testimoni</p>
                    <h2>APA KATA ALUMNI</h2>
                </div>
                <div class="owl-carousel owl-carousel1 owl-theme">
                    <?php foreach ($testimoni as $t) { ?>
                        <div>
                            <div class="card text-center">
                                <div class="image-container">
                                    <div class="video">
                                        <img class="card-img-top" src="<?php echo $this->ppaatthh . 'testimoni/' . $t->file ?>" alt="<?php echo $this->ppaatthh . 'testimoni/' . $t->file ?>" />
                                        <button type="button" class="btn-play" data-bs-toggle="modal" data-video-src="<?php echo $t->link ?>" data-bs-target="#videoModal">
                                            <span></span>
                                        </button>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <h5><?php echo $t->name ?> <br /></h5>
                                    <p class="card-text">
                                        “ <?php echo mb_strimwidth($t->text, 0, 150, "...") ?> ”
                                    </p>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                </div>
                <div class="d-flex justify-content-center">
                    <a href="<?php echo site_url('testimoni') ?>" class="btn btn-selengkapnya3">Lihat semua</a>
                </div>
            </div>

        </div>
    </section>
    -->
    <!-- End of testimonial Section -->

    <!-- quotes Section -->
    <section class="quotes">
        <h1>“Kita tidak mengingat hari,</h1>
        <h1>kita mengingat momen”</h1>
    </section>

    <!-- End quotes Section -->


    <!-- information -->

    <section id="carouselExampleControls" class="card-information-carosel" style="background-color:white;">
        <div class="wrapper-body">
            <div class="title-section">
                <div class="col-lg-12 d-flex justify-content-between">
                    <div class="col-lg-6 ">
                        <p class="textto">Information</p>
                    </div>
                    <div class="col-lg-6 d-flex justify-content-end ">
                    </div>
                </div>
                <div class="col-lg-12">
                    <h1>CARI INFORMASI DISINI</h1>
                    <div class="row d-flex justify-content-center">
                        <div class="col-md-6 col-6 text-right">
                            <button class="btn mb-3 mr-1 carousel-control-prevv" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="prev">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256">
                                    <path fill="currentColor" d="M168.49 199.51a12 12 0 0 1-17 17l-80-80a12 12 0 0 1 0-17l80-80a12 12 0 0 1 17 17L97 128Z" />
                                </svg>
                            </button>
                            <button class="btn mb-3 carousel-control-nextt" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="next">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256">
                                    <path fill="currentColor" d="m184.49 136.49l-80 80a12 12 0 0 1-17-17L159 128L87.51 56.49a12 12 0 1 1 17-17l80 80a12 12 0 0 1-.02 17Z" />
                                </svg>
                            </button>
                        </div>
                        <div class="col-md-6 col-6 text-right">
                            <a href="<?php echo site_url('information'); ?>" type="button" class="btn btn-selengkapnya">
                                Semua Informasi<svg class="carets1" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 256 256">
                                    <path fill="currentColor" d="m180.24 132.24l-80 80a6 6 0 0 1-8.48-8.48L167.51 128L91.76 52.24a6 6 0 0 1 8.48-8.48l80 80a6 6 0 0 1 0 8.48Z"></path>
                                </svg>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- looping start -->
            <div class="carousel">
                <div class="slider carousel-inner">
                    <?php foreach ($information as $t) { ?>
                        <div class="carousel-items me-3" onclick="showDetail(<?php echo $t->id ?>)" style="cursor:pointer">
                            <div class="card">
                                <a href="<?php echo base_url(); ?>Information/detail/?id=<?php echo $t->id ?>"><img src=" <?php echo $this->ppaatthh . 'information/' . $t->image ?>" class="card-img-information" alt="info" />
                                </a>
                                <div class="card-body ">
                                    <div class="d-flex justify-content-between">
                                        <p class="title-category"><?php echo $t->category_name ?></p>
                                        <p class="card-title"><?php echo $t->date ?></p>
                                    </div>
                                    <p class="card-text">
                                        <?php echo $t->title ?>
                                    </p>
                                    <div class="d-flex justify-content-between">
                                        <a class="seemore" href="<?php echo base_url(); ?>Information/detail/?id=<?php echo $t->id ?>">
                                            <p>see more</p>
                                        </a>
                                        <p><i class="bi bi-eye pe-1"></i> <?php echo $t->count ?> kali dilihat</p>
                                    </div>

                                </div>
                            </div>
                        </div>
                    <?php } ?>

                </div>
            </div>
            <!-- looping ended -->
        </div>
    </section>

    <!-- information -->


    <!-- ======= Donation Section 2 ======= -->
    <section id="donation" class="donation">
        <div class="wrapper-body">
            <div class="donationtext">
                <div class="row">
                    <div class="col"><img src="<?= base_url() ?>assets/qr.png" class="img-fluid" alt="qris ctarsa" /></div>
                    <div class="col">
                        <p class="donasip">Donasi</p>
                        <h2>SISIHKAN UNTUK DAPAT LEBIH</h2>
                        <p class="donasis">Kontribusi Anda akan membuka pintu untuk harapan, pendidikan, dan perubahan positif dalam kehidupan mereka yang membutuhkan.
                            Donasi dapat disalurkan langsung pada QRIS yang tertera, atau bisa transfer rekening CT ARSA <strong>01074 001142 1000 </strong> (Bank Mega)
                        </p>
                    </div>
                </div>

            </div>

    </section>
    <!-- End donation Section 2 -->

    <!-- ======= Donation Section ======= -->
    <!-- <section id="donation" class="donation">
        <div class="wrapper-body">
            <div class="donationtext">
                <p class="donasip">Donasi</p>
                <h2>SISIHKAN UNTUK DAPAT LEBIH</h2>
                <p class="donasis">Kontribusi Anda akan membuka pintu untuk harapan, pendidikan, dan perubahan positif dalam kehidupan mereka yang membutuhkan.</p>
            </div>
            <div class="donations">
                <div>
                    <div class="card mb-3">
                        <div class="col-12 kegiatan d-flex justify-content-center align-items-center">
                            <p>Donasi Kegiatan</p>
                        </div>
                        <div class="row g-0">
                            <div class="mega col-6">
                                <img class="p-4" src="<?php echo base_url(); ?>assets/image/mega.png" alt="" />
                            </div>
                            <div class="don3 col-1"></div>
                            <div class="rekening col-5">
                                <p class="norek">324425266521234</p>
                                <p class="atasnama">CTARSA FOUNDATION</p>
                            </div>
                        </div>
                        <div class="rules col-12 d-flex justify-content-center">
                            <p>Pastikan melebihkan 100 rupiah</p>
                        </div>
                        <div class="example col-12 d-flex justify-content-center">
                            <p>ex. Rp.50.100 / Rp.300.100</p>
                        </div>
                        <div class="col-12 d-flex justify-content-center">
                            <button type="button" class="btn btn-qris" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                <img src="<?php echo base_url(); ?>assets/image/image 47.png" alt="" />
                            </button>
                            <button class="btn-qris1">
                                <div class="row">
                                    <div class="copy col-6 d-flex justify-content-center">
                                        <svg style="width: 70%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" id="copy">
                                            <path d="M28,10H12a2,2,0,0,0-2,2V28a2,2,0,0,0,2,2H28a2,2,0,0,0,2-2V12A2,2,0,0,0,28,10ZM12,28V12H28V28Z"></path>
                                            <path d="M7,20H4V4H20V7a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2H7a1,1,0,0,0,0-2Z"></path>
                                        </svg>
                                    </div>
                                    <div class="col-6 d-flex justify-content-center">
                                        <p style="font-size: 8px" class="text-start">
                                            Copy Account Number
                                        </p>
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="card mb-3">
                        <div class="col-12 kegiatan d-flex justify-content-center">
                            <p>Donasi Kegiatan</p>
                        </div>
                        <div class="row g-0">
                            <div class="mega col-6">
                                <img class="p-4" src="<?php echo base_url(); ?>assets/image/mega.png" alt="" />
                            </div>
                            <div class="don3 col-1"></div>
                            <div class="rekening col-5">
                                <p class="norek">324425266521234</p>
                                <p class="atasnama">CTARSA FOUNDATION</p>
                            </div>
                        </div>
                        <div class="rules col-12 d-flex justify-content-center">
                            <p>Pastikan melebihkan 100 rupiah</p>
                        </div>
                        <div class="example col-12 d-flex justify-content-center">
                            <p>ex. Rp.50.100 / Rp.300.100</p>
                        </div>
                        <div class="col-12 d-flex justify-content-center">
                            <button type="button" class="btn btn-qris" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                <img src="<?php echo base_url(); ?>assets/image/image 47.png" alt="" />
                            </button>
                            <button class="btn-qris1">
                                <div class="row">
                                    <div class="copy col-6 d-flex justify-content-center">
                                        <svg style="width: 70%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" id="copy">
                                            <path d="M28,10H12a2,2,0,0,0-2,2V28a2,2,0,0,0,2,2H28a2,2,0,0,0,2-2V12A2,2,0,0,0,28,10ZM12,28V12H28V28Z"></path>
                                            <path d="M7,20H4V4H20V7a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2H7a1,1,0,0,0,0-2Z"></path>
                                        </svg>
                                    </div>
                                    <div class="col-6 d-flex justify-content-center">
                                        <p style="font-size: 8px" class="text-start">
                                            Copy Account Number
                                        </p>
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="card mb-3">
                        <div class="col-12 kegiatan d-flex justify-content-center">
                            <p>Donasi Kegiatan</p>
                        </div>
                        <div class="row g-0">
                            <div class="mega col-6">
                                <img class="p-4" src="<?php echo base_url(); ?>assets/image/mega.png" alt="" />
                            </div>
                            <div class="don3 col-1"></div>
                            <div class="rekening col-5">
                                <p class="norek">324425266521234</p>
                                <p class="atasnama">CTARSA FOUNDATION</p>
                            </div>
                        </div>
                        <div class="rules col-12 d-flex justify-content-center">
                            <p>Pastikan melebihkan 100 rupiah</p>
                        </div>
                        <div class="example col-12 d-flex justify-content-center">
                            <p>ex. Rp.50.100 / Rp.300.100</p>
                        </div>
                        <div class="col-12 d-flex justify-content-center">
                            <button type="button" class="btn btn-qris" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                <img src="<?php echo base_url(); ?>assets/image/image 47.png" alt="" />
                            </button>
                            <button class="btn-qris1">
                                <div class="row">
                                    <div class="copy col-6 d-flex justify-content-center">
                                        <svg style="width: 70%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" id="copy">
                                            <path d="M28,10H12a2,2,0,0,0-2,2V28a2,2,0,0,0,2,2H28a2,2,0,0,0,2-2V12A2,2,0,0,0,28,10ZM12,28V12H28V28Z"></path>
                                            <path d="M7,20H4V4H20V7a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2H7a1,1,0,0,0,0-2Z"></path>
                                        </svg>
                                    </div>
                                    <div class="col-6 d-flex justify-content-center">
                                        <p style="font-size: 8px" class="text-start">
                                            Copy Account Number
                                        </p>
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="card mb-3">
                        <div class="col-12 kegiatan d-flex justify-content-center">
                            <p>Donasi Kegiatan</p>
                        </div>
                        <div class="row g-0">
                            <div class="mega col-6">
                                <img class="p-4" src="<?php echo base_url(); ?>assets/image/mega.png" alt="" />
                            </div>
                            <div class="don3 col-1"></div>
                            <div class="rekening col-5">
                                <p class="norek">324425266521234</p>
                                <p class="atasnama">CTARSA FOUNDATION</p>
                            </div>
                        </div>
                        <div class="rules col-12 d-flex justify-content-center">
                            <p>Pastikan melebihkan 100 rupiah</p>
                        </div>
                        <div class="example col-12 d-flex justify-content-center">
                            <p>ex. Rp.50.100 / Rp.300.100</p>
                        </div>
                        <div class="col-12 d-flex justify-content-center">
                            <button type="button" class="btn btn-qris" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                <img src="<?php echo base_url(); ?>assets/image/image 47.png" alt="" />
                            </button>
                            <button class="btn-qris1">
                                <div class="row">
                                    <div class="copy col-6 d-flex justify-content-center">
                                        <svg style="width: 70%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" id="copy">
                                            <path d="M28,10H12a2,2,0,0,0-2,2V28a2,2,0,0,0,2,2H28a2,2,0,0,0,2-2V12A2,2,0,0,0,28,10ZM12,28V12H28V28Z"></path>
                                            <path d="M7,20H4V4H20V7a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2H7a1,1,0,0,0,0-2Z"></path>
                                        </svg>
                                    </div>
                                    <div class="col-6 d-flex justify-content-center">
                                        <p style="font-size: 8px" class="text-start">
                                            Copy Account Number
                                        </p>
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="card mb-3">
                        <div class="col-12 kegiatan d-flex justify-content-center">
                            <p>Donasi Kegiatan</p>
                        </div>
                        <div class="row g-0">
                            <div class="mega col-6">
                                <img class="p-4" src="<?php echo base_url(); ?>assets/image/mega.png" alt="" />
                            </div>
                            <div class="don3 col-1"></div>
                            <div class="rekening col-5">
                                <p class="norek">324425266521234</p>
                                <p class="atasnama">CTARSA FOUNDATION</p>
                            </div>
                        </div>
                        <div class="rules col-12 d-flex justify-content-center">
                            <p>Pastikan melebihkan 100 rupiah</p>
                        </div>
                        <div class="example col-12 d-flex justify-content-center">
                            <p>ex. Rp.50.100 / Rp.300.100</p>
                        </div>
                        <div class="col-12 d-flex justify-content-center">
                            <button type="button" class="btn btn-qris" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                <img src="<?php echo base_url(); ?>assets/image/image 47.png" alt="" />
                            </button>
                            <button class="btn-qris1">
                                <div class="row">
                                    <div class="copy col-6 d-flex justify-content-center">
                                        <svg style="width: 70%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" id="copy">
                                            <path d="M28,10H12a2,2,0,0,0-2,2V28a2,2,0,0,0,2,2H28a2,2,0,0,0,2-2V12A2,2,0,0,0,28,10ZM12,28V12H28V28Z"></path>
                                            <path d="M7,20H4V4H20V7a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2H7a1,1,0,0,0,0-2Z"></path>
                                        </svg>
                                    </div>
                                    <div class="col-6 d-flex justify-content-center">
                                        <p style="font-size: 8px" class="text-start">
                                            Copy Account Number
                                        </p>
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="card mb-3">
                        <div class="col-12 kegiatan d-flex justify-content-center">
                            <p>Donasi Kegiatan</p>
                        </div>
                        <div class="row g-0">
                            <div class="mega col-6">
                                <img class="p-4" src="<?php echo base_url(); ?>assets/image/mega.png" alt="" />
                            </div>
                            <div class="don3 col-1"></div>
                            <div class="rekening col-5">
                                <p class="norek">324425266521234</p>
                                <p class="atasnama">CTARSA FOUNDATION</p>
                            </div>
                        </div>
                        <div class="rules col-12 d-flex justify-content-center">
                            <p>Pastikan melebihkan 100 rupiah</p>
                        </div>
                        <div class="example col-12 d-flex justify-content-center">
                            <p>ex. Rp.50.100 / Rp.300.100</p>
                        </div>
                        <div class="col-12 d-flex justify-content-center">
                            <button type="button" class="btn btn-qris" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                <img src="<?php echo base_url(); ?>assets/image/image 47.png" alt="" />
                            </button>
                            <button class="btn-qris1">
                                <div class="row">
                                    <div class="copy col-6 d-flex justify-content-center">
                                        <svg style="width: 70%" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" id="copy">
                                            <path d="M28,10H12a2,2,0,0,0-2,2V28a2,2,0,0,0,2,2H28a2,2,0,0,0,2-2V12A2,2,0,0,0,28,10ZM12,28V12H28V28Z"></path>
                                            <path d="M7,20H4V4H20V7a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2H7a1,1,0,0,0,0-2Z"></path>
                                        </svg>
                                    </div>
                                    <div class="col-6 d-flex justify-content-center">
                                        <p style="font-size: 8px" class="text-start">
                                            Copy Account Number
                                        </p>
                                    </div>
                                </div>
                            </button>
                        </div>
                    </div>
                </div>
            </div>
            <button class="prevbutton"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256">
                    <path fill="currentColor" d="M165.66 202.34a8 8 0 0 1-11.32 11.32l-80-80a8 8 0 0 1 0-11.32l80-80a8 8 0 0 1 11.32 11.32L91.31 128Z" />
                </svg>
            </button>
            <button class="nextbutton">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 256 256">
                    <path fill="currentColor" d="m181.66 133.66l-80 80a8 8 0 0 1-11.32-11.32L164.69 128L90.34 53.66a8 8 0 0 1 11.32-11.32l80 80a8 8 0 0 1 0 11.32Z" />
                </svg>
            </button>
        </div>
    </section>-->
    <!-- End donation Section -->


    <!-- Modal donation -->
    <!--<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="popupedit">
                    <div class="container">
                        
                    </div>
                </div>
            </div>
        </div>
    </div>-->

    <!-- Modal testimoni -->
    <div class="modal fade" id="videoModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content rounded-0">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Youtube Video</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- 16:9 aspect ratio -->
                    <div class="ratio ratio-16x9">
                        <iframe class="embed-responsive-item" src="" id="video" allowfullscreen allowscriptaccess="always" allow="autoplay"></iframe>
                    </div>
                </div>
            </div>
        </div>
    </div>

</main>